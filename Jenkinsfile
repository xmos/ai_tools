                ...
                stage("Build linux runtime") { steps {
                    ...
                    script {
                        docker.image('tensorflow/build:2.14-python3.9').inside("-e SETUPTOOLS_SCM_PRETEND_VERSION=${env.TAG_VERSION} -v ${env.WORKSPACE}:/ai_tools -v .bazel-cache:/.cache -w /ai_tools") {
                            dir("xformer") {
                                sh "bazel build //:xcore-opt --verbose_failures --linkopt=-lrt --crosstool_top='@sigbuild-r2.14-clang_config_cuda//crosstool:toolchain' --//:disable_version_check --remote_cache=${env.BAZEL_CACHE_URL}"
                            }
                            dir("python") {
