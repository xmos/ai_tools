...
typedef struct {
  ...
  struct {
    ...
    /**
     * `input[k].offset` is the offset @math{z_k} applied to elements of
     * @tensor{x_k}. This is usually derived from the zero-point of the
     * quantized vector @tensor{x_k}.
     */
    int16_t offset;
  ...
  struct {
    /**
     * `output.multiplier` is the scale factor @math{m_{out}} applied when
     * quantizing output elements of @tensor{y}.
     *
    ...
    /**
     * `output.offset` is the offset @math{z_{out}} applied when quantizing
     * output elements of @tensor{y}.
     */
    int16_t offset;
  } output;
} nn_add_params_t;
 ...
 *
 * @f[
 *      v_i[k] \leftarrow  sat_{16}\!\left(round\!\left(sat_{16}\!\left(
 * sat_{16}\!\left( floor\!\left(\frac{x_i[k]}{2^{s_i}}\right)
 *              \right)+z_i\right)\cdot\frac{m_i}{2^{14}} \right)\right) \\
 *
 *      y[k] \leftarrow
 * round\!\left(\frac{sat_{16}\!\left(sat_{16}\!\left(round\!\left(\frac{m_{out}}{2^{14}}\cdot
 * sat_{16}\!\left(v_0[k]
 *                          + v_1[k]\right)\right)\right) +
 * z_{out}\right)}{2^{8}}\right)
 *
 * @f]
 *
 * where
 *
 * @par
 * each @tensor{v_i} is an intermediate 16-bit vector representing the contents
 * of @tensor{x_i} requantized according to the contents of `params`,
 *
 * @par
 * @math{k} is the output index,
 *
 * @par
 * @math{sat_{16}\!\left(\cdot\right)} saturates its arguments to @math{16}-bit
 * bounds, and
 *
 * @par
 ...
 * @tensor_shape{N}.
 *
 * `params` describes how the elements of each vector should be treated in
 * accordance with their quantization parameters.
 *
 ...
 */
void bsign_8_prepare(nn_bsign_8_plan_t* plan, nn_bsign_8_job_t* jobs,
 ...
 */
void bsign_8(bnn_b32_t* Y, const int8_t* X, const nn_bsign_8_plan_t* plan,
             const nn_bsign_8_job_t* job);

void bsign_8_ref(bnn_b32_t* Y, const int8_t* X, const nn_bsign_8_plan_t* plan,
                 const nn_bsign_8_job_t* job);
