...
This application should print around 300 lines ending with::

  [...]
  Human (98%)

The printed lines provide profiling information which will be explained in
detail below.  This line in the CMakeLists.txt enables profiling::

  -DTFLMC_XCORE_PROFILE

This example shows how profiling can reduce execution time from 74 to 8
milliseconds.

Purpose of profiling
--------------------

Profiling allows the designer to understand how fast the network runs,
and which parts of the network take the bulk of the time.  If some parts of
the network take too much time, they may be remedied by changing the network,
or by suggesting an optimisation for this type of network to the team at XMOS.

How to enable profiling
-----------------------

Profiling is controlled by a ``#define``.  By defining the
``TFLMC_XCORE_PROFILE`` symbol, the application will print profiling
information.
...
Typically, the last two contain the most interesting information.

All times are printed in 10 ns ticks (0.000,000,01 second), and summarised in
milliseconds.

* Per node profiling lists the number of ticks for each node in the graph
...
* Initialisation and preparation prepare the graph and the tensor-arena,
  respectively, for the inferencing step.  Typically initialisation and
  preparation occur once before running many inferences.  For example, for a
  keyword spotter one would initialise and prepare the tensor arena once, and
  then perform inferences time and time again.

* Inferencing time is the time taken to, given some input data, infer the
  output.

Note that the times given below may differ from those you obtain depending on
your target, the precise version of the AI tools, and the version of the
underlying compiler.
...
The first lines list initialisation per node. 320 ticks (3.2 us) for node 0,
536 ticks (5.36 us) for node 1, etc.  The second section shows that all
OP_XC_PAD operators together took 7503 ticks (75 us) to initialise.  The
final section shows that initialisation altogether took 1 ms.

In a similar vein, the next three sections show the preparation of the tensor
arena, which takes a total of 1.71 ms.

The last three sections will be something like::
...
The bottom line shows the total time for the invocation of this network:
74.86ms; the lines above it shows in detail where the time went.  Note the
two lengthiest parts of the inferencing::

  27    OP_XC_conv2d_v2                  2916236      29.16ms
  2     OP_DEPTHWISE_CONV_2D             4487819      44.88ms

The first number of each line shows the number of times that the network
invoked the operator.  The second column shows the operator name.  The third
column shows the number of ticks, and the final column shows the time in
milliseconds.

The ``OP_XC_conv2d_v2`` operator executed 27 times, so on average took just
over one millisecond per invocation.  The ``OP_DEPTHWISE_CONV_2D`` operator
took 22.44 ms per invocation.  These operations take so much time for two
reasons: the ``OP_DEPTHWISE_CONV_2D`` operator is a reference implementation
and not optimised for the XCORE, and neither operation has taken advantage of
parallelisation.

Any operator that has ``XC`` (for XCORE) in the name has been optimised for the
XCORE.  Reference implementation operators do not have ``XC`` in their name.
The reason that the network uses the reference implementation can be seen in a
warning emitted by the graph transformer::
...
The total execution time has reduced by 44 milliseconds.  The network used the
optimised convolution twice more than previously, accounting for an extra 556
...
Execution time has gone down from 30.55 to 8 milliseconds.  Five threads have
yielded a 3.8x speedup on this occasion; typically on larger graphs the
speedup is closer to 5x.
