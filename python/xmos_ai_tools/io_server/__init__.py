...
class xmos_io_server(ABC):
    ...
    def write_input_tensor(self, raw_img, tensor_num = 0, model_num = 0):
        ...
        self._download_data(IOSERVER_TENSOR_RECV_INPUT, raw_img,
                                tensor_num = tensor_num, model_num = model_num)

   
    def read_output_tensor(self, length, tensor_num = 0, model_num=0):

        # Retrieve result from device
        data_read = self._upload_data(IOSERVER_TENSOR_SEND_OUTPUT, length,
                                          model_num = model_num, tensor_num = tensor_num)
