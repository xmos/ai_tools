...
PARAMS["default"].update({
    "non_matching_reduction_dims": [
        t for t in itertools.chain(itertools.product(range(4)),
...
PARAMS["smoke"].update({
    "reduction_dims": PARAMS["default"]["reduction_dims"]
})
PARAMS["smoke"].update({
    "non_matching_reduction_dims": [
        t for t in itertools.chain(itertools.product(range(1, 4)),
...
def test_non_matching_reduction_dims(trf_pass, input_shape,
                                     non_matching_reduction_dims):
    model = build_mean(input_shape=input_shape,
                       reduction_dims=non_matching_reduction_dims)
    _test_non_matching_params(trf_pass, model)
