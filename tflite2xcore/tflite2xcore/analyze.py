...
def calc_subgraph_mem_req(subgraph):
    ...
    while operators:
        ...
        op_mem_reqs[op.name] = {
            'buffer_tensors': {
                tensor.name: len(tensor.buffer.data) for tensor in coexisting
                             if len(tensor.buffer.data)
            },
            'arena_tensors': {
                tensor.name: tensor.size for tensor in coexisting
        ...
        coexisting = {
            tensor for tensor in coexisting
            if ([consumer for consumer in tensor.consumers if consumer in operators]
    ...
    return {
        ...
        'buffers': sum(
            len(buffer.data) for buffer in subgraph.model.buffers
            if [owner for owner in buffer.owners
                if owner in subgraph.tensors]
        ),
...
def analyze_model(model):
    analysis = {
        'subgraphs': [
            calc_subgraph_mem_req(subgraph) for subgraph in model.subgraphs
        ]
    }
    analysis['buffers'] = sum(
        len(buffer.data) for buffer in model.buffers
    )
    analysis['arena'] = max(
