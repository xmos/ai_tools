...
class layerArtifacts:
    ...
    def createXcoreModel(self):
        ...
        xf.convert(
            "./tflite_models/{}.tflite".format(self.layerID),
            "./xcore_models/{}.tflite".format(self.layerID),
            params=None,
        )
    ...
    def evaluateTflite(self):
        ...
        interpreter = tf.lite.Interpreter(
            model_path="./tflite_models/{}.tflite".format(self.layerID)
        )
        ...
        for datapoint in self.repDataset:
            ...
            interpreter.invoke()
            self.tfliteOutputs.append(
                interpreter.get_tensor(out["index"]).astype(np.float32)
            )
    ...
    def evaluateXcore(self):
        ...
        interpreter = TFLMInterpreter(
            model_path="./xcore_models/{}.tflite".format(self.layerID)
        )
        ...
        for datapoint in self.repDataset:
            ...
            interpreter.invoke()
            self.xcoreOutputs.append(
                interpreter.get_output_tensor(0).astype(np.float32)
            )
    ...
    def calcErrors(self, data1, data2, findError=False):
        print(
            "Calculating {} {} errors for layer: ".format(data1, data2)
            + str(self.layerID)
        )
        ...
        if data1 == "tf":
            if data2 == "tflite":
                (
                    self.tf_tflite_error,
                    self.tf_tflite_abserror,
                    self.tf_tflite_maxabs,
                    self.tf_tflite_mse,
                    self.tf_tflite_hist,
                ) = calcErrorSet(self.outputs, self.tfliteOutputs, findError)

            elif data2 == "xcore":
                (
                    self.tf_xcore_error,
                    self.tf_xcore_abserror,
                    self.tf_xcore_maxabs,
                    self.tf_xcore_mse,
                    self.tf_xcore_hist,
                ) = calcErrorSet(self.outputs, self.xcoreOutputs, findError)
        elif data1 == "tflite" and data2 == "xcore":
            (
                self.tflite_xcore_error,
                self.tflite_xcore_abserror,
                self.tflite_xcore_maxabs,
                self.tflite_xcore_mse,
                self.tflite_xcore_hist,
            ) = calcErrorSet(self.tfliteOutputs, self.xcoreOutputs, findError)
    ...
    def errorHists(self):
        ...
        hist3 = genHist(
            self.tflite_xcore_hist, self.layerID, "tflite_xcore", self.opList
        )
...
def genHist(histogram, layer, hist_type, opList):
    ...
    ax.set_title(
        "{} layer {} Average Error Histogram (ops: {})".format(
            hist_type, layer, str(opList)
        )
    )
...
def mainGraphs(Layers, error_type):
    ...
    xax = range(0, 35)
    rects1 = ax.bar(
        xax, tf_tflite_err, width, label="tf - tflite {}".format(error_type)
    )
    rects2 = ax.bar(
        [i + width for i in xax],
        tf_xcore_err,
        width,
        label="tf - xcore {}".format(error_type),
    )
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(
        xax, tflite_xcore_err, width, label="tflite - xcore {}".format(error_type)
    )
...
def generateBaseModel(alpha=0.25, inputDim=224):
    model = tf.keras.applications.mobilenet.MobileNet(
        alpha=alpha,
        input_shape=(inputDim, inputDim, 3),
        weights="imagenet",
        classes=1000,
    )
    return model
...
def tfLayerArtifacts(model):
    ...
    for i in range(1, len(model.layers)):
        ...
        if (
            ("_bn" in model.layers[i].name)
            or ("_relu" in model.layers[i].name)
            or ("dropout" in model.layers[i].name)
        ):
...
def setRepDatasets(Layers, samples):
    ds = tfds.load("imagenet_v2", split=("test"), with_info=False, as_supervised=False)
