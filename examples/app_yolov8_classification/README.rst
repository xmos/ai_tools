...
path used by this example, including a Python 3.11-compatible ``onnx2tf``
version. Run the requirements install after installing ``xmos-ai-tools`` so
these bounds override any incompatible transitive packages selected by the base
tools installation.
...
or installed a different ``onnx2tf`` version, rerun
``python -m pip install -r requirements.txt`` before generating the model.
...
Lite conversion with ``onnx2tf`` before invoking ``xcore-opt``. This avoids
passing hybrid TFLite models to TFLite Micro.
