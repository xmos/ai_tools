...
struct KernelCommand {
    ...
    size_t stack_words;
    void* stack;
    KernelCommand(kernel_function_t f, void* a=nullptr, size_t w=0, void* s=nullptr)
    ...
    // Uses a std::vector of std::thread to dispatch kernel funnctions to SW threads.
    KernelDispatcher::KernelDispatcher(bool use_current) : 
        use_current_(use_current), stack_ptr_(nullptr) {}
    KernelDispatcher::~KernelDispatcher() {}
...
XCoreStatus KernelDispatcher::Add(kernel_function_t function, void* argument, size_t stack_words) {
    assert(stack_ptr_);
