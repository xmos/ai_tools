...
void test_bnn_conv2d_bin_out_asm_directed() {
#define H_STRIDE 1
#define V_STRIDE 1
#define H_OFFSET 0
#define V_OFFSET 0
...
#define CHAN_WORDS_OUT ((CHANS_OUT + 32 - 1) / 32)
  ...
  // plan.x_width_loop_counter); printf("%p\n", &plan);
  bnn_conv2d_bin_out_asm((bnn_b32_t*)&Y, (bnn_b256_t*)&X, (bnn_b256_t*)&K,
                         thresholds, &plan);
  // printf("done\n");
  nn_bnn_conv2d_bin_out_plan_t plan_ref;
  bnn_conv2d_bin_out_init(&plan_ref, &x, &y, &k);
  bnn_conv2d_bin_out((bnn_b32_t*)&Y_ref, (bnn_b256_t*)&X_ref,
                     (bnn_b256_t*)&K_ref, thresholds, &plan_ref);
...
void test_bnn_conv2d() {
  ...
  RUN_TEST(test_bnn_conv2d_bin_out_asm_directed);
