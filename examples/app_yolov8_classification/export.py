...
def _check_installed_dependencies():
    ...
    onnx2tf_version = version("onnx2tf")
    if onnx2tf_version != "1.29.0":
        raise RuntimeError(
            "This example requires onnx2tf==1.29.0 for Python 3.11 compatibility. "
            f"Found onnx2tf=={onnx2tf_version}; run `python -m pip install -r requirements.txt` before rerunning."
        )
...
def _write_onnx2tf_test_data():
    data = Path(SAMPLE_IMAGE_PATH).read_bytes()
    image = np.frombuffer(data, dtype=np.uint8).reshape(1, HEIGHT, WIDTH, 3).astype(np.float32)
    np.save(ONNX2TF_TEST_DATA_PATH, np.repeat(image, 20, axis=0))
...
def _convert_onnx_to_int8_tflite(onnx_model_path):
    ...
    with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as calibration_data_file:
        ...
        try:
            ...
            if "tflite_backend" in inspect.signature(onnx2tf.convert).parameters:
                convert_kwargs["tflite_backend"] = "tf_converter"
