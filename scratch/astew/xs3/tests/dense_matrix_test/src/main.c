    ...
    int32_t * acc)
{
    ...
    for(int i = 0; i < VPU_INT8_ACC_PERIOD; i++){
        // printf("acc[%d] = ", i);
        // printf("0x%02x%02x%02x%02x\n", vD[2*i+1],vD[2*i],vR[2*i+1],vR[2*i]);
        acc[i] = (int32_t) (0
            | vD[2*i+1] << 24
            | vD[2*i+0] << 16
            | vR[2*i+1] <<  8
...
static void compute_chunk_int8_c(
    const int8_t* W, 
    const int8_t* x, 
    int32_t* acc)
{
    for(int i = VPU_INT8_ACC_PERIOD-1; i >= 0; i--){
        for(int k = 0; k < VPU_INT8_EPV; k++){
            int32_t c = W[k];
            int32_t d = x[k];
            int64_t tmp_acc = acc[i] + c*d;

            if(tmp_acc > VPU_INT32_MAX)
                tmp_acc = VPU_INT32_MAX;
            if(tmp_acc < VPU_INT32_MIN)
                tmp_acc = VPU_INT32_MIN;

            acc[i] = (int32_t) tmp_acc;
        }
...
static void compute_tile_int8_c(
    const int8_t* W,
    const int8_t* x,
    ...
    int32_t* acc)
{
    for(int chunk = 0; chunk < N_chunks; chunk++){
        compute_chunk_int8_c(W, x, acc);
        x += VPU_INT8_EPV;
        W += VPU_INT8_EPV * VPU_INT8_ACC_PERIOD;
    }
}


static void mat_vec_mul_int8_c(
    const int8_t* W,
    const int8_t* x,
    ...
    int8_t* y)
{
    ...
    for(int bi = 0; bi < N_bands; bi++){
        compute_tile_int8_c(W, x, N_chunks, accp);
        W = &W[N_chunks * VPU_INT8_EPV * VPU_INT8_ACC_PERIOD];
        accp += VPU_INT8_ACC_PERIOD;
    }

    //Go back and bring it down to 8 bits
    for(int i = 0; i < N_bands*VPU_INT8_ACC_PERIOD; i++){
        y[i] = ((acc[i] >> (shr[i]-1)) + 1) >> 1;
    }
}
...
#define N (1 * VPU_INT8_EPV)
#define M (1 * VPU_INT8_ACC_PERIOD)
static void _test_compute_chunk_int8()
{
    int32_t WORD_ALIGNED c_acc[M];
    int32_t WORD_ALIGNED asm_acc[M];
...

    {
        ...
        compute_chunk_int8_c(W, x, c_acc);
        compute_chunk_int8_asm(W, x);

        extract_acc_int8(asm_acc);

        for(int i = 0; i < M; i++){
            TEST_ASSERT_EQUAL_INT32(exp_res[i], c_acc[i]);
            TEST_ASSERT_EQUAL_INT32(c_acc[i], asm_acc[i]);
        }
    }
...
#define N (96)
#define M (16)
#define N_chunks (N / VPU_INT8_EPV)
static void _test_compute_tile_int8()
{
...

    {
        for(int d = -10; d < 10; d++){
            ...
            compute_tile_int8_c((int8_t*) W, x, N_chunks, c_acc);
            compute_tile_int8_asm((int8_t*) W, x, N_chunks);
...
#define M (32)
#define N_chunks (N / VPU_INT8_EPV)
#define N_bands  (M / VPU_INT8_ACC_PERIOD)
static void _test_matrix_mul_int8()
{
...

    {
        ...
        int32_t exp_res[M] = {-54201,-59419,11433,36351,-109939,-7542,-33520,28053,-18583,-44674,56499,-47063,-87584,115857,-83856,-120622,
                               25948,-22516,-20404,53874,22575,-9217,-54528,-2189,-63700,85961,-4248,-30772,7254,-16077,47916,-18935};
        ...
        mat_vec_mul_int8_c(W, x, N_bands, N_chunks, shr, c_acc, c_y);
        mat_vec_mul_int8_asm(W, x, N_bands, N_chunks, shr, asm_y);

        for(int i = 0; i < M; i++){
            char str_buff[100];
            sprintf(str_buff, "(output index %u)", i);
            // printf("%d\t%ld\t%ld\t\t%d\t%d\n", i, exp_res[i], c_acc[i], exp_res8[i], c_y[i] );
            TEST_ASSERT_EQUAL_INT32_MESSAGE(exp_res[i], c_acc[i], str_buff);
...
#define N_chunks (N / VPU_INT8_EPV)
#define N_bands  (M / VPU_INT8_ACC_PERIOD)
static void _test_matrix_mul_int8_timing()
{
...

    {
        ...
        unsigned t = timerEnd();
        printf("Time taken: %d ns (%u iterations)\n", t*10, iters);

        printf("Average time: %f ns\n", (t/((float)iters)) * 10);

        printf("Instruction estimate: %u\n", 12 + N_bands * (5 + N_chunks * 19));
    }
}
#undef N
#undef M
...
int main(void)
{
  ...
  RUN_TEST(_test_compute_chunk_int8);
  RUN_TEST(_test_compute_tile_int8);
  RUN_TEST(_test_matrix_mul_int8);
  RUN_TEST(_test_matrix_mul_int8_timing);
