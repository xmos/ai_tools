        ...
        if os.path.isdir(directory):
            ...
            if os.path.isfile(flatbuffer_xcore):
                input_files = glob.glob(
                    os.path.join(directory, "test_data/model_xcore/*.x")
                )
                model = read_flatbuffer(flatbuffer_xcore)
                input_quantization = model.subgraphs[0].outputs[0].quantization

                flatbuffer_stripped = os.path.join(
                    directory, "models/model_stripped.tflite"
                )
                output_files = glob.glob(
                    os.path.join(directory, "test_data/model_stripped/*.y")
                )
                ...
                for input_file, output_file in zip(
                    sorted(input_files), sorted(output_files)
                ):
                    test_cases.append(
                        {
...
def test_XC_conv2d_shallowin(test_model_app, XC_conv2d_shallowin_test_case, abs_tol):
    if is_xfail(XC_conv2d_shallowin_test_case):
        pytest.xfail()
    assert run_test_case(test_model_app, XC_conv2d_shallowin_test_case, abs_tol)
