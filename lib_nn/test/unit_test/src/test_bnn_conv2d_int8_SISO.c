...
void make_thresholds(float * post_activation_multiplier, float * post_activation_bias, 
  ...
  for (unsigned ch=0;ch < chans_out;ch++){
    float input_range = receptive_volume;

    unsigned range =  rand()%10;
    float r =  (float)rand() / (float)RAND_MAX;
    float output_range = (range/2) + (float)(1<<range)*r;

    post_activation_multiplier[ch] =( output_range/input_range)/2;
...
static void run_int8_config(int8_t* Y_p, int8_t* Y_ref_p, bnn_b32_t* X_ref,
  ...
  quantise_activation(
               ...
               &accu_shr, &final_shr, receptive_volume, chan_overlaps);
...
#define CHAN_WORDS_IN DIV_BY_AND_ROUND_UP(CHANS_IN, 32)
  ...
  bnn_b32_t WORD_ALIGNED K[CHANS_OUT*K_HEIGHT*K_WIDTH*CHAN_WORDS_IN + NN_BCONV2D_KERNEL_OVERRUN_WORDS];
  bnn_b32_t WORD_ALIGNED X_ref[X_HEIGHT*X_WIDTH*CHAN_WORDS_IN+8];
  int8_t WORD_ALIGNED Y[Y_HEIGHT][Y_WIDTH][CHANS_OUT];
  int16_t WORD_ALIGNED post_activation_multiplier_q[CHANS_OUT+64];
  int16_t WORD_ALIGNED post_activation_bias_q[CHANS_OUT+64];
            ...
            for (unsigned x_width = k_width; x_width <= MAX_X_WIDTH;
              ...
              for (unsigned chans_in = MIN_CHANS_IN; chans_in <= MAX_CHANS_IN;
                ...
                for (unsigned chans_out = MIN_CHANS_OUT;
                  ...
                  bnn_b32_t *X_ref =(bnn_b32_t *)malloc(sizeof(bnn_b32_t)*(x_height*x_width*chan_words_in+7));
...
void test_bnn_conv2d_int8_out_SISO_sub_image(){
  ...
  // bnn_b32_t WORD_ALIGNED K[MAX_CHANS_OUT*FULL_K_HEIGHT*FULL_K_WIDTH*MAX_CHAN_WORDS_IN  +8*12];
  // int8_t WORD_ALIGNED Y[FULL_Y_HEIGHT][FULL_Y_WIDTH][MAX_CHANS_OUT];
  // int16_t WORD_ALIGNED post_activation_bias_q[MAX_CHANS_OUT+32];
  // int16_t WORD_ALIGNED post_activation_multiplier_q[MAX_CHANS_OUT+32];
  // bnn_b32_t WORD_ALIGNED data_scratch[FULL_K_HEIGHT * FULL_K_WIDTH * MAX_CHAN_WORDS_IN + 
  //   NN_BCONV2D_KERNEL_OVERRUN_WORDS]; 
  ...
  for(unsigned chans_out = MIN_CHANS_OUT; chans_out <= MAX_CHANS_OUT; chans_out += 4){
    for(unsigned chans_in = MIN_CHANS_IN; chans_in <= MAX_CHANS_IN; chans_in += 32){
      ...
      int8_t *Y = (int8_t *) malloc(sizeof(int8_t) * FULL_Y_HEIGHT*FULL_Y_WIDTH*chans_out);
      
      bnn_b32_t * K= (bnn_b32_t *)malloc(sizeof(bnn_b32_t)*( chans_out*FULL_K_HEIGHT*FULL_K_WIDTH*chan_words_in +92));
            ...
            for (unsigned y_loc_y = 0; y_loc_y<y.height; ++y_loc_y){
              for (unsigned y_sub_width = 1; y_sub_width<y.width-y_loc_x; ++y_sub_width){
                for (unsigned y_sub_height = 1; y_sub_height<y.height-y_loc_y; ++y_sub_height){
                    ...
                    printf("%u %u %u %u %u %u %u %u\n", chans_out, chans_in, h_stride, v_stride, y_loc_x, y_loc_y, y_sub_width, y_sub_height);
