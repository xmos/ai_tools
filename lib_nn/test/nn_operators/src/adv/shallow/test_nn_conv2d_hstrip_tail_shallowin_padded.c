...
#if USE_ASM(nn_conv2d_hstrip_tail_shallowin_padded)
 #define HAS_ASM (1)
#else
 #define HAS_ASM (0)
#endif
...
void test_nn_conv2d_hstrip_tail_shallowin_padded_case0()
{
    ...
    for(int v = start_case; v < N_casses && v <= stop_case; v++){
        ...
        for(int C_out = 4; C_out <= CHANS_OUT_MAX; C_out += 4){
            ...
            nn_conv2d_hstrip_tail_shallowin_padded_c((nn_image_t*) Y_c, (nn_image_t*) X, K_init, (nn_bss_block_t*) &bss, 
            ...
            nn_conv2d_hstrip_tail_shallowin_padded_asm((nn_image_t*) Y_asm, (nn_image_t*) X, K_init, (nn_bss_block_t*) &bss, 
                                            K_h, K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
                                            x_v_stride, y_params.channels, y_params.width, zero_point_vec, C_out);
#endif
    ...
    for(int v = start_case; v < N_casses && v <= stop_case; v++){
        ...
        for(int C_out = 4; C_out <= CHANS_OUT_MAX; C_out += 4){
            ...
            for(unsigned pad_t = 0; pad_t < 3; pad_t++){
                ...
                PRINTF("\t\t\t\tC...\n");
                memset(Y_c, 0xCC, sizeof(Y_c));
                nn_conv2d_hstrip_tail_shallowin_padded_c((nn_image_t*) Y_c, X_patch_start, K_init, (nn_bss_block_t*) &bss, 
                                                K_h, K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
                                                x_v_stride, y_params.channels, y_params.width, zero_point_vec, C_out);
#endif
#if TEST_ASM
                PRINTF("\t\t\t\tASM...\n");
                memset(Y_asm, 0xCC, sizeof(Y_asm));
                nn_conv2d_hstrip_tail_shallowin_padded_asm((nn_image_t*) Y_asm, X_patch_start, K_init, (nn_bss_block_t*) &bss, 
                                                K_h, K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
                                                x_v_stride, y_params.channels, y_params.width, zero_point_vec, C_out);
#endif
